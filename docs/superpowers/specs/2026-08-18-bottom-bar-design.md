...
给 pake 壳加一条浮在网页之上的底部工具栏，三个按钮：后退、刷新、设置。
...
| 形态 | 悬浮浮层：居中、圆角矩形、毛玻璃底、白色图标（**见下方「视觉定稿」**） |
...
## 视觉定稿（2026-08-18，设计画布上敲定）

初版按「不透明深色胶囊」实现，随后在设计画布上比过四个方向
（Slate / Docked / Frosted / Vibrant），选定 **Frosted**，并把形状从全圆胶囊
改成圆角矩形、尺寸整体放大。最终值：
...
| 底色 | `0x8C1C1C1E`（深色玻璃，55% 不透明度） |
| 描边 | `0x26FFFFFF` |
| 投影 | `0x33000000`，offset (0,3)，blur 14 |
| 图标 | 24，白色；禁用 `white24` |
| 触控格 | 56×56，`spaceEvenly` 均分（三格 168，四个间隙各 10） |
| 距底 | 手势条 inset + 8 |

**只有这一套配色，不跟随网页深浅。** 壳拿不到可靠信号判断当前页面是深是浅
——唯一能读的是网页的 `<meta name="theme-color">`，很多站点根本不给。目标
站点多是深色，所以钉死深色玻璃 + 白图标；**浅色网页上对比度会下降，这是选
毛玻璃就得接受的代价**，不是待修的缺陷。

投影必须挂在 `ClipRRect` **外面**（里面画的会被裁掉），而 `BackdropFilter`
必须被 `ClipRRect` 包住（否则它去模糊整个屏幕）——两层裁剪半径要一致。

## 架构

```
packages/pake_shell/lib/src/bottom_bar.dart          新：无状态浮层
...
条宽钉死 208 之后，三个 56 的触控格加起来 168 必须塞得进去。塞不进去
（比如有人把格子调大到 72）会直接触发 `RenderFlex` 溢出，`bottom_bar_test`
里任何一个 pump 用例都会失败——不需要额外写断言去防。

但「塞得进去」和「点得准」是两回事：为了塞进去而把格子缩到 44 以下，测试
一样是绿的。所以单独有一条用例钉住 44 这个移动端触控下限。
...
`offset` 给 2 而不是 1：摆法 B 让浮层离底边有一段距离，只移动 1 倍高度它还有
...
本身，以及浮层只有 208 宽（比通栏窄，两侧的网页按钮还能点）。
