...
- **用外部浏览器打开当前页**
  ...
  `url_launcher` 的 `launchUrl(mode: LaunchMode.externalApplication)` 一行的事，入口放
  底部栏或设置页，和「复制当前 URL」那条挨着做正好。
...
- **分享**
  影视站这类内容是拿来传的：看到一部片想丢给朋友，现在只能截图。`share_plus` 调系统分享
  面板，带上当前页的标题和 URL（是**当前页**，不是构建时那个首页 URL——和「复制当前 URL」
  取的是同一份数据）。顺带可以分享 app 本身：4kvm 这种靠口口相传的，分享一条 Releases
  链接比让人自己找有用。

  反向的「别的 app 分享链接进来打开」不做——单站壳收到任意链接没地方放，同域链接又本来
  就能点开。

  **这三条得一起想**：分享、复制当前 URL、用外部浏览器打开，加上已有的回主页，都是同一个
  位置的入口。而底部栏是钉死的 208 宽三个按钮（后退/刷新/设置），再塞就得先把它改成
  「更多」菜单——先决定这件事，再决定往里放几条。
