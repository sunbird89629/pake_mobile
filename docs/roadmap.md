...
- **深链与下载分流**（原 `safeDomains` 窄化）
  普通 http/https 导航一律壳内开——单站壳「主站跳到源站」必须留在壳内，cookie 和上下文才
  连续，不为分流而分流。只拦两类确凿开不好的：非 http(s) scheme 的深链（`taobao://`、
  `alipays://`、`intent://`）在 `shouldOverrideUrlLoading` 里看 `uri.scheme` 丢系统；
  下载在 `onDownloadStartRequest` 丢浏览器/下载管理器。被 WebView 拒的登录页走
  「用外部浏览器打开当前页」那条逃生口，不在这里重复。
...
- **视频下载**
  和「内置视频播放器」共用同一套嗅探层（hook `<video>` 的 src / `shouldInterceptRequest`
  抓直链），嗅到之后除了「用内置播放器打开」，再加一个「下载到本地」入口。

  按直链类型分三档，工作量差很多：**mp4 直链**——直接下，带 WebView 的 cookie / UA /
  Referer 过防盗链，最简单；**m3u8/HLS**——下全部分片再合并，AES-128 加密的要先解 key，
  是主要工作量；**DRM / 自定义加密流**——下不了，干净退出不骗人。

  落盘走 Android MediaStore（分区存储）存到 Movies / Download，不自己申请宽泛目录权限；
  下载进度给个入口看（debug drawer 或底部菜单），支持失败重试。

  合规：下载站点视频可能违反 ToS 或版权，做成用户显式触发的操作、不做自动后台下载，
  风险自担并在文档里写清楚。
...
- 忽略证书错误 —— 安全风险。
- 拖拽上传 —— 移动端几乎没有拖拽场景。
- **远程配置/脚本热更**：注入脚本、UA 从远端拉，不重装 APK 就生效。
